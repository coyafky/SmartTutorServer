...
// TutorProfileSchema.index({ 'teachingExperience.subjects.name': 1 });
  ...
  async getPostById(postId) {
    const post = await TutoringRequest.findById(postId);
  ...
  async getReportedPosts(queryParams) {
    ...
    const total = await TutoringRequest.countDocuments({ 'reports.0': { $exists: true } });
  ...
  async getPostStatistics() {
    ...
    const publishedPosts = await TutoringRequest.countDocuments({ status: 'published' });
    const pendingPosts = await TutoringRequest.countDocuments({ status: 'pending' });
    const rejectedPosts = await TutoringRequest.countDocuments({ status: 'rejected' });
  ...
  async getMatchStatistics() {
    ...
    const successfulMatches = await Match.countDocuments({ status: 'successful' });
