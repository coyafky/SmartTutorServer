...
function generateUniqueRequestId(parentId) {
  ...
  const timestamp = (baseTimestamp + Math.floor(globalCounter / 100)).toString();
  // 使用全局计数器的后两位作为毫秒部分
...
async function generateTutoringRequest(parent, existingIds = new Set()) {
  ...
  const city = parent.location?.city || faker.helpers.arrayElement(CHINESE_CITIES);
  const district = parent.location?.district || faker.helpers.arrayElement(DISTRICTS[city] || DISTRICTS['北京']);
  const coordinates = parent.location?.coordinates?.coordinates || generateChinaCoordinates(city);
  ...
  return {
    ...
    preferences: {
      ...
      teachingStyle: faker.helpers.arrayElements(
        ['互动式教学', '启发式教学', '项目式学习', '翻转课堂', '传统讲授', '个性化辅导'],
        { min: 1, max: 3 }
      ),
      budget: {
...
async function processBatch(parents, batchIndex, batchSize, totalRequests, maxPerParent) {
  ...
  console.log(`\n处理批次 ${batchIndex + 1}，生成 ${batchSize_actual} 个请求（总进度: ${startIndex}/${totalRequests}）`);
  ...
  while (currentCount < batchSize_actual && parentIndex < parents.length) {
    ...
    for (let i = 0; i < requestCount && currentCount < batchSize_actual; i++) {
      try {
        const requestData = await generateTutoringRequest(parent, existingRequestIds);
        if (requestData) {
          ...
          if (currentCount % 10 === 0 || currentCount === batchSize_actual) {
            process.stdout.write(`\r已生成: ${currentCount}/${batchSize_actual}`);
          }
        }
      } catch (error) {
  ...
  try {
    if (requestsToCreate.length > 0) {
      ...
      console.log(`批次 ${batchIndex + 1} 成功插入 ${requestsToCreate.length} 条记录`);
      console.log(`批次处理耗时: ${((endTime - startTime) / 1000).toFixed(2)}秒`);
    ...
    if (error.writeErrors) {
      ...
      console.error(`\n批次 ${batchIndex + 1} 部分插入成功: ${successCount}/${requestsToCreate.length}`);
      console.error(`错误数量: ${error.writeErrors.length}，第一个错误: ${error.writeErrors[0]?.errmsg || '未知错误'}`);
      return successCount;
    } else {
...
// 主函数：批量生成家教需求请求
async function generateTutoringRequestsBatch() {
  ...
  try {
    ...
    console.log(`平均速度: ${(totalCreated / totalTimeSeconds).toFixed(2)} 条/秒`);
